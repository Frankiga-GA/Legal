  ...
  return (
    <div className="p-8 md:p-12 min-h-screen bg-brand-black relative">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <header className="mb-20 flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-white/[0.05] pb-10">
          <div className="space-y-3">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-brand-gold/10 rounded-full border border-brand-gold/20">
              ...
              <span className="text-[9px] uppercase tracking-widest text-brand-gold font-bold">Arquitectura Neuronal v2</span>
            </div>
            ...
            <p className="text-brand-accent/40 font-light text-sm tracking-wide">Despliegue y gestión de agentes legales especializados con memoria indexada.</p>
          </div>
          <button 
            onClick={() => setIsCreating(true)}
            className="flex items-center gap-3 px-8 py-4 bg-brand-gold text-brand-black rounded-2xl hover:bg-white transition-all font-bold tracking-tight shadow-[0_10px_30px_rgba(197,160,89,0.2)] group"
          >
            <Plus className="w-5 h-5 group-hover:rotate-90 transition-transform" />
            Desplegar Nuevo Nodo
          </button>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {bots.map((bot) => (
            <div key={bot.id} className="glass-card p-10 rounded-3xl group relative flex flex-col h-full bg-white/[0.01] border border-white/[0.05] hover:border-brand-gold/30 transition-all shadow-2xl">
              <div className="flex justify-between items-start mb-10">
                <div className="p-4 rounded-2xl bg-brand-gold/10 text-brand-gold group-hover:bg-brand-gold group-hover:text-brand-black transition-all">
              ...
              <div className="flex items-center justify-between pt-8 border-t border-white/[0.05]">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[10px] uppercase tracking-widest text-brand-gold font-bold">{bot.docs} Activos</span>
                </div>
                <button 
                  onClick={() => setActiveBot(bot)}
                  className="px-6 py-3 rounded-xl bg-brand-ivory text-brand-black text-[11px] font-bold uppercase tracking-widest hover:bg-white transition-all shadow-lg"
                >
                  Invocar
                </button>
              </div>
        ...
        {isCreating && (
          <div className="fixed inset-0 bg-brand-black/90 backdrop-blur-xl z-50 flex items-center justify-center p-6">
            <div className="bg-brand-dark border border-white/[0.05] rounded-3xl shadow-2xl w-full max-w-xl p-10 animate-in fade-in zoom-in duration-500">
              ...
              <form onSubmit={handleCreateBot} className="space-y-6">
                <div className="space-y-2">
                  <label className="text-[10px] uppercase tracking-widest text-brand-accent/60 font-semibold">Nombre del Nodo</label>
                  <input 
                    type="text" 
                    value={newBot.name}
                ...
                <div className="space-y-2">
                  <label className="text-[10px] uppercase tracking-widest text-brand-accent/60 font-semibold">Contexto Operacional</label>
                  <textarea 
                    value={newBot.description}
                    onChange={(e) => setNewBot({...newBot, description: e.target.value})}
                    placeholder="Define los parámetros legales y áreas de experiencia..." 
                ...
                <div className="space-y-2">
                  <label className="text-[10px] uppercase tracking-widest text-brand-accent/60 font-semibold">Base de Conocimiento</label>
                  <div className="border border-dashed border-white/[0.1] rounded-xl p-10 text-center hover:bg-white/[0.02] cursor-pointer transition-all group">
                    ...
                    <p className="text-xs text-brand-accent/40 font-light">Sube documentos (PDF, DOCX) para indexar en la memoria del nodo</p>
                  </div>
                </div>
                <div className="flex gap-4 pt-6">
                  <button type="submit" className="flex-1 py-4 bg-brand-ivory text-brand-black rounded-xl hover:bg-white font-bold tracking-tight transition-all">Inicializar Nodo</button>
                </div>
              </form>
            </div>
          </div>
